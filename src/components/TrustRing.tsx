    ...
    <div className={`relative inline-flex flex-col items-center gap-1 ${className}`}>
      ...
      >
        <svg width={outer} height={outer} className="transform -rotate-90">
          <defs>
            <linearGradient id="trust-ring-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="var(--c2k-accent-primary)" />
              <stop offset="100%" stopColor="var(--c2k-accent-secondary)" />
            </linearGradient>
          </defs>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          {children ?? (
            <div className="w-1/2 h-1/2 rounded-full bg-c2k-bg-elevated flex items-center justify-center">
              <span className="text-[10px] font-medium text-c2k-text-muted">{score}</span>
            </div>
          )}
        </div>
      </button>
      {/* Trust bar – thin frame under avatar, fill = score */}
      <div className="h-1 rounded-full bg-c2k-bg-elevated overflow-hidden" style={{ width: outer }}>
        <div
          className="h-full rounded-full bg-c2k-accent-primary transition-all"
      ...
      {showBreakdown && isExpanded && (
        <>
          ...
          <div className="absolute left-full ml-2 top-0 z-50 min-w-[200px] py-2 px-3 bg-c2k-bg-card border border-white/10 rounded-xl shadow-c2k-soft">
            <p className="text-xs font-semibold text-c2k-text-muted uppercase mb-2">Trust Score: {score}</p>
            <ul className="space-y-1">
              {SEGMENT_LABELS.map(({ key, label }, i) => (
                <li key={key} className="flex items-center justify-between gap-4 text-sm">
                  <span className="text-c2k-text-secondary">{label}</span>
                  <span className="font-medium text-white">{values[i]}%</span>
                </li>
              ))}
            </ul>
          </div>
